...
type TradeConsumer = (trade: Trade) => void
...
function generateTrade(trade: any, consumer: TradeConsumer, ctx: LootContext) {
	const wants = trade.wants ?? []
	if (wants.length == 0) return
	const wants0 = generateItem(wants[0], ctx)
	let wants1 = undefined
	if (wants.length > 1) {
		wants1 = generateItem(wants[1], ctx)
	}
	const givesArr = trade.gives ?? []
	if (givesArr.length == 0) return
	const gives = generateItem(givesArr[0], ctx)
	consumer({wants0, wants1, gives})
}

function generateItem(item: any, ctx: LootContext): ItemStack {
	const compositeFunction = composeFunctions(item.functions ?? [])
	let itemStack = new ItemStack(Identifier.parse(item.item ?? 'air'), computeInt(item.quantity ?? 1, ctx))
	compositeFunction(itemStack, ctx)
	return itemStack
}
